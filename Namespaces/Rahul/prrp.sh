...
sudo ip lin add veth4 type veth peer name veth5
...
sudo ip netns exec router2 ip addr add 10.0.1.1/24 dev veth3

sudo ip netns exec router1 ip addr add 10.1.1.2/24 dev veth4
...
sudo ip netns exec router1 ip link set dev veth4 up
...
sudo ip netns exec peer1 ip route add default via 10.0.0.2
sudo ip netns exec peer2 ip route add default via 10.0.1.2
sudo ip netns exec router1 ip route add default via 10.1.1.2
sudo ip netns exec router2 ip route add default via 10.1.1.1
